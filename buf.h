...
typedef enum __BufState { BUF_STATE_DIRTY = 0, BUF_STATE_CLEAN = 1 } BufState;

typedef enum __BufList { BUF_LIST_DIRTY = 0, BUF_LIST_CLEAN = 1 } StateList;
 ...
 */
extern void GetBufInfoByListNum(StateList listnum, Buf **ppBufInfo,
                                int *pNumBuf);
 ...
 * "ppBufInfo" that can contain the buffers.
 */
extern void GetBufInfoInBufferList(int hashEntNum, Buf **ppBufInfo,
                                   int *pNumBuf);
